...
#include "include/sht11.h" //Importacio de la llibreria del sensor de temperatura
#include <time.h>
#include <stdio.h>
#include <string.h>

BOOL bucleInicial = TRUE;
//Boolea que serveix per indicar si estem dormits o no. TRUE-> hem rebut DD i estem dormits.
BOOL dd = FALSE; 
//Enter que controla els segons del nostre Duty Cicle. Si es 10, es reinicia
int count;
count = 0;
//Variable amb la que es realitza la lectura de la temperatura
UINT16 temperatura;
...
#define RF_CHANNEL		15  //El nostre canal
#define TX_PERIOD       50  // Packet sent each n'th cycle

#define PANID			0x2420
#define MYADDR			0x999 //La nostre adreca
#define DESADDR			0xABAC //La adreca de desti del nostre coordinador
...
int main(void) {
    ...
    //Bucle "inutil" que envia un sol cop la peticio de conexio a la xarxa del coordinador
    //A la memoria se explica el perque d'aquest while (al final del apartat 3)
    while (1 == 1) {
        ...
        pTxBuffer[0]=  0xAB; //AB significa en el nostre grup la solicitud de conexio
        pTxBuffer[1] = status;
        pTxBuffer[2] = nRecv<<8;
    ...
    //Switch que coordina les diferents accions un cop conectats.
    while (1 == 1){

    	//Aquesta comprovacio no funciona. Si es vol comprovar el funcionament
    	//de la temperatura, s'ha de treure el switch i enviar directament aquesta
        switch(rfSettings.pRxInfo->pPayload[0]) { 

            case 0xDD: //Al rebre DD es mira si estem desperts o dormits

            	if (dd) { //Si estem dormits (dd = TRUE) hem de canviar el boolea
            		//El proces de despertarnos es realitza a l'interrupcio!
            		dd = FALSE;
            	} else { //Si estem desperts ens dormim
                	basicRfReceiveOff(); //Apaguem el transceiver
                	LPM3; //Reduim el nivell d'energia
                	dd = TRUE;
                }
                break;

            case 0xBB: //Al rebre BB es realitza l'enviament de la temperatura
                temperatura = sht11_temp(); //Per llegir el registre que guarda la temperatura
                temperatura = (temperatura * D2_14BIT) + D1_CEL30; //Per fer el pas a Celsius

                //Es realitza l'enviament de la trama amb la temperatura
                UINT8 status;
                FASTSPI_UPD_STATUS(status);
                ...
                pTxBuffer[0]=  0xBB; //BB per indicar el tipus de trama
                pTxBuffer[1] = temperatura; //El valor de la temperatura
                pTxBuffer[2] = status;
                pTxBuffer[3] = nRecv<<8;
                pTxBuffer[4] = (UINT8)(nRecv & 0x00FF);
...
__interrupt void WDT_Interval(void){
    ...
    //SI ES VOL IMPLEMENTAR LA FUNCIO "DORMIR":

    /*if (dd) { //Mirem si ens volem adormir 
        count = count+1; 
        if (count == 10){ //Si estem al segon 10, mirem si ens envien algo (despertar)
                LPM3_EXIT; //Canviem el nivell d'energia a normal
                basicRfReceiveOn(); //Activem el transceiver
                count = 0; //Reset
        }
    }*/
}
